...
Prof Andy Baker in the UNSW School of Biological, Earth & Environmental Sciences uses stalagmite growth rates to study patterns of rainfall. As stalagmite growth rates depend on the rate of water flow into the soil, records of growth can be used to reconstruct rainfall variability through time.
...
Andy asked us to cross check his results, your goal is to reproduce some key figures from the 2021 paper.

**Do these visualisations in your `Wk1-2-stalagmites.qmd` document**

> In some code chunks, we've given you some partially completed R code, these are hints, complete the rest of the code before you run them
...
The code below gets you started. You'll need to fill in the spaces indicated by XXX.
...
stalagmites_data |>
  ...
  theme_classic() +
  labs(x = "Annual lamina growth rate (mm)", y = "Number") +
  xlim(c(0, 1.2))
```
...
transformed_stalagmites_data |>
  filter(entity_id == 85) |>
  ggplot(aes(x = lam_age_bp_0_1950ad, y = log10_1_lamina_thickness_mm)) +
  geom_line() +
  labs(x = "Years BP", y = expression("Growth rate (log, mm yr^{-1})")) +
  theme_classic()
```
...
**Question: Did you notice any errors in the dataset? There is one row that isn't data, which is it?**
