...
stalagmites_data <- stalagmites_data |>
  filter(lamina_thickness_mm < XXX)
```
...
transformed_stalagmites_data |>
  ...
  scale_y_log10() +
  theme_classic() +
  labs(x = "Years BP", y = "Growth rate (mm/yr)") +
...
**Question: Did you notice any errors in the dataset? There is one row that isn't data, which is it? [ANSWER IN MOODLE]**
