...
Read in the "plantHeightSingleSpp" dataset and check the structure.

```{r}
data <- read_csv("data/plant_height/plantHeightSingleSpp.csv")
str(data)
```
...
```{r}
# subset only data you want 
data1 <- data %>% 
  select(height, lat, alt)
```
...
Use hist() to graph the distribution of height from our plant height data set.

```{r}
ggplot(data1, aes(height)) + geom_histogram()
...
```{r}
ggplot(data1, aes(log_height)) + geom_histogram()
```
...
```{r}
ggplot(data1, aes(lat, log_height))+ 
  geom_point() + 
  geom_smooth(method="lm")
```
...
```{r}
ggplot(data1, aes(alt, log_height))+ 
  geom_point() + 
  geom_smooth(method="lm")
...
So our data implies that plants tend to be shorter further from the equator and in higher altitudes!
