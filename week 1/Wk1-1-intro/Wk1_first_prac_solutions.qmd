...
Noe you try installing the packages: `report`, `modelbased`, and `performance`
...
Function: A block of code that performs a specific task. For example, `library()` is a function that we use to load packages.

Argument: An input that a function needs to perform it's task. Many argument inputs are already embedded into a function as a default that can be changed as needed. For example, `verbose = FALSE` is a default argument embedded into the `library()` package. If you wanted to view the output messages of running the function library(), you can add this into the parentheses.

```{r}
library(tidyverse, verbose = TRUE)
```

Object/Variable (The words can be used interchangeably but for clarity I will refer to these as objects): A memory location used to store information/values. Many things can be saved in objects: numbers, lists, tables, and data frames along with various other types of data.
...
# multiple strings can be saved into the same object
weather <- c("Sun", "Rain", "Partly Cloudy") # create object
weather #view object 
class(weather) # what type of object is this? 
```

**Numeric** (num) : Any numeric value with or without a decimal becomes a numeric data type (e.g. `1.2`, `4`, `34.7`, `54`, etc.) T
...
A good way to do this is by looking at the **structure** of your data set with **View()** and **str()**
...
We can see here that our data is in the tibble data frame format. Our data has 5 variables, 4 of which are numeric (or double). This is correct as they contain decimals. Our last variable is a factor variable.
...
```{r}
#| echo: false
dat$Species <- as.character(dat$Species) # changing variable type to chr 
```

The way you would assign your variable as a factor would be with the `as.factor()` function

```{r}
dat$Species <- as.factor(dat$Species) # changing variable type to factor
```

### Visualizing your data

A good way to better understand your data is by creating figures that show general patterns and trends.

For this we will use the package `ggplot2`
...
ggplot(data = dat, aes(x = Species, y = Sepal.Length, color = Species)) +
...
You can also look at patterns of other traits by switching out your y-axis for the trait you're interested in. Let's try it!

Can you figure out which species have the widest petals?

```{r}
ggplot(data = dat, aes(x = Species, y = Petal.Width, color = Species)) +
  geom_boxplot() # create boxplot
```
...
# note: I'm writing over the iris dataset 
```
...
```{r}
dat$`lam_age (BP, 0=1950AD)` <- as.integer(dat$`lam_age (BP, 0=1950AD)`)
```
...
```{r}
dat$entity_id <- as.factor(dat$entity_id)
dat$seq_id <- as.factor(dat$seq_id)
...
First, read in the data and assign it to the object "dat1".

```{r}
dat1 <- read_csv("data/sydneybeaches.csv")
```
...
dat1$Enterococci_cfu_per_100ml <- as.integer(dat1$Enterococci_cfu_per_100ml)
str(dat1)
```

Use this format to visualize the amount of Enterococci bacteria found at each Site. (hint: replace your_data, x_axis, and y_axis)

```{r}
ggplot(dat1, aes(Site, Enterococci_cfu_per_100ml)) +
         geom_boxplot() +
