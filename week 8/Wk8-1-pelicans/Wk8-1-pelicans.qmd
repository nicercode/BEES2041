...
For this, we will use the real data collected from Roxanne. You should have already imported the data from `data/trainingData.csv` and divided it into training and test data sets.
...
peli_labelled_train_top_vars <- peli_labelled_train |> select(is_pelican, any_of(top_vars))

# Fit model
fit_rf_top <- ranger(is_pelican ~ ., data = peli_labelled_train_top_vars)
