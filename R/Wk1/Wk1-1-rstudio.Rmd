...
![Opening `Wk_1_first_prac.qmd`](open-qmd.png)
...
**Importantly** you only need to install packages **ONCE**. This is different to **loading** packages into R using the `library()` function. You do this once for each Quarto doc right at the start of the document.
...
- The object that holds the dataset is `staglamite_data`

> Its crucial to assign the output of `read_csv()` into an object, otherwise R won't hold it in memory for you to use

**Notice:** In the "Environment" tab, we can now see `staglamite_data` that we just created!
...
Now you are all set you proceed with the prac!
