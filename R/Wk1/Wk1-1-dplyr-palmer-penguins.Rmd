...
Before diving into data wrangling, I want to introduce to you a very *special* operator, the pipe! `%>%`. Importantly, the pipe pairs well with `dplyr` functions as it allows to do multiple data wrangling steps in one go. 
...
Then adapt their code there to answer the following.

<h4>Extracting rows</h4>

*Try:* Use the function `filter()` to extract all penguins from `Dream` island
