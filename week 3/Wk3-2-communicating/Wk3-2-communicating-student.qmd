...
5.  Click on the `Wk3-2-communicatings.Rproj` to open the RStudio project and you're in!!!
...
We can make the legend nicer for a formal report with further tweaking using the `colour` and `shapes` argument. We have to specify the title for both colour and shapes because those are the settings we set to vary by `island`.
...
Typically in scientific publications, figures usually have the figure legend to describe the contents so adding a title is not necessary, but if you would like, you can create it with the `title` argument.
...
If you want your table output to print nicely in your notebook, we can enlist the extra package `kableExtra`. We'll use `parameters` to tidy our model output and `kableExtra` to decorate your table so it looks nice in the end. You will need to install `kableExtra` if you want to do the following. 
