...
Linear models are one of the most powerful and widely used tools in statistics and data science. At their core, they help us understand how y varies with one or more predictors $(x_1, x_2, ...)$.

In the previous practical, we learned about population means and how natural variation in the process of sampling creates uncertainty. We also learned that we can estimate a confidence interval around the mean, within which we believe the true mean to occur.
...
![Image credit:  @allison_horst](images/you-can-do-it.png){width=80%} <br>
<br>
...
We will be working with packages from the `tidyverse` and `easystats`, building on skills from previous pracs. You should have these already installed on your machines.
...
Let's read in Isaac's data. This is in your `"data/"` folder. 
...
![Linear model components and different slope values, Image credit: Data Dan](images/slope-explainer.png){width=80%} <br>
...
The coefficient for the slope is `r cloze("log10_rainfall", c("(Intercept)", "log10_rainfall"))` and the coefficient value is `r cloze("-0.543846", c("3.805973", "-0.543846", "<2e-16"))`.

2. What direction is the slope coefficient, what does this tell us?
...
For a nicer output you can also use `parameters(towers_fit)`. This function conveniently gives us the **95% confidence intervals** of our coefficients.
...
`[-0.56, -0.52]` provides a range of values between which the `r cloze("true", c("sample", "true"))` population parameter coefficient most likely lies. 


## Variation explained by the model (R^2 and F-statistic)

![Relationship with data, *R^2* and *F*-statistic, Image credit: Data Dan](images/r2-and-f-value.png){width=60%}

<br>
> There are two metrics we use to assess our model fit
...
- A higher R^2 value indicates a strong relationship between the y variable (`log10_leaf_mass_per_area`) and the x variable(s) (`log10_rainfall`), but *importantly* it **does not imply causation**.
...
- The **F-statistic's p-value** indicates how likely we are to see a value of this magnitude in a population where there is no meaningful effect.
...
The `r cloze("R-squared value", c("F-statistic", "R-squared value", "P value"))` indicates a `r cloze("moderate", c("weak", "moderate", "strong"))` amount of variance in the y variable (`log10_leaf_mass_per_area`) is explained by the x variable (`log10_rainfall`). 

The `r cloze("F-statistic", c("F-statistic", "R-squared value", "P value"))` suggests that the predictor (`log10_leaf_mass_per_area`) explains a `r cloze("significant", c("non-significant", "significant"))` amount of variance in `log10_leaf_mass_per_area`.

## *F*-statistic and *P*-values - how likely is it to get our results by chance?

> In linear models, the *p*-value is derived from the F-distribution

![Relationship between R^2, *F* and *P*, Image credit: Data Dan](images/f-value-p-value.png){width=60%}
<br>

The *p*-value and the *F*-statistic are closely related. In the last prac, we learned to calculate **probability (p-value)** for a *t*-statistic from a standard normal distribution. We can do the same for our *F*-statistic from the *F*-distribution!
...
> In other words, in our hypothetical world, finding an effect is REALLY rare - meaning in the real world there is likely something interesting happening.
...
Generally, as the *F*-statistic increases, the `r cloze("p-value", c("the intercept", "the slope", "p-value"))` decreases. A large *F*-statistic means that it's `r cloze("unlikely", c("likely", "unlikely"))` to be sampled by chance. 
...
Okay that's enough theory for the day... let's have a go at estimating the means and confidence intervals for our y variable (`log10_leaf_mass_per_area`) for a range of values of x (`log10_rainfall`) using our linear model. We use `estimate_means(towers_fit, by = "log10_rainfall")` for this.
...
**Notice** how we store the output of our `ggplot` in a variable called `towers_model_predictions`; this gives us the option to add to the plot incrementally later.
...
- `geom_point(size = 2, col = "red", alpha = 0.2, stroke = 0)` - plotting the raw data using points
    ...
    - `stroke` controls the thickness of the border of the points (so here we want no border)
...
- `geom_line(data = means_towers, aes(y = Mean), col = "#EB8240", linewidth = 1)`, plot the line for our model predictions
  ...
  - `col = "#EB8240"`, set a custom colour using [hexadecimal (a computer dialect to represent colour)](https://g.co/kgs/NJcxHVJ)
...
> Let's get you to try run a linear model, interpret the output and create a plot with a familiar dataset! 

# LM continuous (Penguins)

## Example 2 - Bringing back the Palmer penguins for allometry

![We can learn so much from these cuties. Image credit: @allison_horst](images/palmer_penguins.png){width=60%}
<br>
...
> We want to try to answer the question: **How does bill length scale with body mass in Palmer Penguins?**
...
- Create a data visualisation showing off the `penguins` data and your predicted means from your linear
model
...
parameters(penguins_fit) |> tibble() # For unrounded values in the confidence intervals
...
There is a `r cloze("positive", c("positive", "negative"))` relationship between bill length and body mass.
...
3. Yikes, it looks like rounding the confidence intervals for the slope in the output of `parameters()` is not very helpful. Try `parameters(penguins_fit) |> tibble()` to look at the unrounded values of the confidence intervals. What do the confidence intervals for the slope tell us? 

The confidence intervals for the slope tell us that the true population mean for the slope likely lies between
...
- Generate the summary output for your model using `summary()` and `parameters()` 
  - Make some interpretations from this output using the: 
      - slope
      - confidence interval of the slope
      - R^2 value
      ...
      - p-value
- Generate some model predictions from your model using `estimate_means()`
- Create a data visualisation showing off the `airquality` data and your predicted means from your linear model
  - Plot the raw data with `geom_point()`
  - Add the confidence intervals with `geom_ribbon()`
  - Add the predicted linear relationship with `geom_line()`
  - Choose some [fun colours](https://sites.stat.columbia.edu/tzheng/files/Rcolor.pdf) if you would like!
...
The true mean estimate of the slope likely exists somewhere between `r cloze("1.97 and 2.89", c("-183.2 and -110.77", "1.97 and 2.89"))`.
...
We are primarily interested in how `smr` - a continuous variable changes with `temp_treatment` - a categorical variable. We can use a linear model to test for **differences in means** in `temp_treatment`. The key difference in this example is that our x variable is a categorical variable.

![Using a linear model with categorial and continuous variables](images/lm-anova.png){width=60%} <br>

Let's fit a model for `smr` using `temp_treatment` as a predictor:
...
> How do we know whether there is an effect of treatment group?
...
- The `anova()` function splits up the total variation in our data into two main components: 
    1. Variation explained by our model predictor(s) (`temp_treatment`). Think of this as a signal for a treatment effect.
    2. Unexplained variation (`Residuals`). Think of this as noise in our data.

## Detecting an overall effect - some maths

![Under the hood, maths behind an ANOVA](images/signal-to-noise.png){width=60%} <br>
...
- **smaller** values mean that we have more **noise** in our data.
...
- In other words, there is a `r cloze("strong", c("strong", "weak"))` signal for a treatment effect and it is very `r cloze("unlikely", c("likely", "unlikely"))` to observe this effect in our hypothetical boring world.
...
![Categorical variables becomes multiple predictors in a linear model](images/lm-multiple-predictors.png){width=80%} <br>
Importantly: 

- `(Intercept)` represents the **mean** of `temp_treatment [20C]`. By default, the `lm()` function sorts the categorical variable alphabetically and then chooses the first value as `(Intercept)`, like this:

```{r}
data_martino$temp_treatment |> unique() |> sort()
```
...
## Estimate pairwise comparisons in means (contrasts)

> What if we want to change the reference group from temp_treatment [20C] to temp_treatment [28C]? 

We can use the `estimate_contrasts()` function to give us pairwise comparisons of the means between all groups. Try it out!
...
**Notice** how we store the output of our `ggplot` in `martino_model_predictions`, this gives us the option to add to plot incrementally later.
...
## Do penguins differ in their flipper length across islands?

![Location, location, location. Image Credit: [Mario Gregorio Saavedra Rodríguez](images/https://rpubs.com/mgsaavedraro/665527)](images/palmer-islands.png){width=75%}
...
# Plot with raw data points
  ...
  geom_errorbar(aes(ymin = CI_low, ymax = CI_high), width = 0, color = "grey", linewidth = 3) + # Confidence intervals
  geom_point(size = 4, color = "orange") + # Estimated means
...
3. Which two islands showed the biggest mean difference in flipper length?
...
The p-value of `r cloze("0.312", c("< .001", "0.312"))` tells us that there is a `r cloze("moderate", c("very low", "moderate", "very high"))` chance that we would observe the mean difference of -1.88 between Torgersen Island and Dream Island if there were `r cloze("no", c("no", "strong"))` differences in flipper length between islands.
...
1. Our `F-statistic` of the `anova()` output is relatively big (34.70), what might this mean? 

A relatively big value of our `F-statistic` means that we have relatively more `r cloze("signal to noise", c("noise to signal", "signal to noise"))` in our data. 

2. Interpret the intercept of your model. What does it represent?

The intercept in our model represents the `r cloze ("mean", c("standard deviation", "mean", "mean square error"))` of spray `r cloze("A", c("A", "B", "C", "F", "D", "E", "F"))`.

3. Which spray type(s) would you recommend in reducing insect counts?

From our pairwise comparisons, I would recommend to use spray(s) `r cloze("C, D or E", c("B, C or F", "C, D or E", "A, B or F"))`.
