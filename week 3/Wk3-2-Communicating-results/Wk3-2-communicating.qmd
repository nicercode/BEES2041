...
![Image credit: Hadley Wickham [R 4 Data Science](images\https://r4ds.hadley.nz/)](images/the-whole-game-communicate.png) <br>


Communication of the results of statistical analyses is usually done by a combination of written text, visual displays of the results, and the inclusion of test statistics to support the written statements.
...
At the end of the exercises, you should be able to:
...
![Image credit:  @allison_horst](images\images/nice-R.png){width=80%}
<br>
...
- presenting the results twice. If a table has some summary statistics (e.g., means and standard deviations of some variables), then there is no need to include these also in the text—you can just refer to the table. Similarly, if you have a figure that displays the means of a variable, you normally don't need to mention those specifically in the results unless you want to highlight the actual values (e.g., “the mean abundance (± s.e.) in summer, 100.5 ± 4.1 m2, was almost twice as high as winter 52.7 ± 3.4 m2”). Normally, you can just let the reader use the figure to see the magnitude of the values and refer to the figure (e.g., the abundance was higher in winter than in summer (Fig. 1)).
...
2. If it is a more complex test, then it is usually more efficient to present the results of that test in a table, and just refer to that table in the text. For example (from the third linear models practical):

The removal of predatory minks from islands in the Baltic Sea resulted in a significant increase in the abundance of voles (Fig.1, Table 1).
...
This text is a good starting point for writing up the results of your analysis. However, you'd need to edit it slightly. For example, you might:

- include the proper names for variables, not the column names from your data frame (e.g., bill length instead of `bill_length_mm`)
- remove the adjusted R² value (usually not relevant)
...
Running `report` directly on fit will give you a summary of the model, including the coefficients, standard errors, t-values, and p-values. However, with a single categorical predictor, people tend to report the result of the ANOVA table.
...
> The result of the linear model suggests that the species differ considerably in their bill length (F(2, 339) = 410.60, p < .001). The effect size was large (r² = 0.71, 95% CI [0.67, 1.00]), indicating that 71% of the variance in bill length was explained by species in this analysis.
...
- just using whatever figure the statistical software program produces as its default - you nearly always need to tidy these up before they are ready for presentation.
- missing axis labels
...
If you want, the X and Y limits can be set with the `xlim` and `ylim` functions.
...
The plotting symbol can be customised by adding code to `geom_point()`. For example, to get large, purple squares, we would use: 
...
Check out the [codes for symbols](https://www.datanovia.com/en/blog/ggplot-point-shapes/) and further information on [Plotting with ggplot: colours and symbols](http://environmentalcomputing.net/plotting-with-ggplot-colours-and-symbols/).
...
We can make the legend a nicer format for a report with further tweaking using the `colour` and `shapes` argument. We have to specify the legend title for both colour and shapes because those are the settings we set to vary by `island`.
...
The size, fonts, and symbols of legends can all be customised if necessary (see [Plotting with ggplot: adding titles and axis names](http://environmentalcomputing.net/plotting-with-ggplot-adding-titles-and-axis-names/)).


**Adding a title**

Typically, scientific publications have the figure legend to describe the contents, so adding a title is not necessary. However, if you would like, you can create one with the `title` argument.
...
You can see that `ggplot` works by adding more and more layers to further customise plots. These can be combined into custom themes that you can reuse many times (see [ggplot: Altering the overall appearance](http://environmentalcomputing.net/plotting-with-ggplot-altering-the-overall-appearance/)). Or, you can use some of the built-in themes that alter many options at once. `theme_bw()` is a handy one that gets rid of the grey background.
...
Faceting is a really powerful tool to create the same plot for different groups. Let's say we want to compare different sexes of penguins here. We will use `facet_wrap()` for this! Remember to tell ggplot what you want to facet by using `~sex`.
```{r}
ggplot(penguins, aes(x = body_mass_g, y = bill_length_mm)) + 
  geom_point(aes(shape = island, color = island)) +
  labs(x = "Body mass (grams)", y = "Bill length (mm)",
...
We can filter out the NA penguins using `filter(!is.na(sex))` or `drop_na(sex)`. We can also change the labels for the `facet_wrap` using `labeller`.
...
Remember that you should always present measures of variation associated with the different levels of a categorical variable. You can do this with boxplots (median + quartiles) or with bar plots that display means +/- error bars (that show standard deviations or standard errors). Have a look at these two figures to understand why.

![](images/example_se.png){width=60%} <br>

Both have the exact same means, but differ in their variance. The one on the left has lower variation within groups relative to the variation between groups, would have a higher value of a test statistic used to contrast the two groups (t in a t-test or F in an ANOVA), and would be more likely to find a significant difference between the groups (lower P-value). Consequently, the interpretation of these two is very different, but it is only possible to get this understanding from the figure if the variation is also shown.


Bar plots with error bars are commonly used, but keep in mind that they have some limitations—in particular, they are unable to show the underlying distribution of the data very effectively. Error bars are presented as equal in size (+ or - from the mean) even when the underlying data are very skewed.


**Create error bars**

Let's create error bars for different species in bill length.

First, we need to get some species means. We can do this from our model we fitted earlier.
...
Let's adjust the arms of the error bars and colour, and make this look nicer:

```{r}
species_bill_length <- 
  ggplot(penguins, aes(x = species, y = bill_length_mm)) + 
  geom_jitter(width = 0.2, alpha = 0.5, stroke = 0, size = 2) + 
  geom_errorbar(data = penguin_means, aes(y = Mean, ymin = CI_low, ymax = CI_high),
                width = 0.2, colour = "blue", linewidth = 1) +
  geom_point(data = penguin_means, aes(y = Mean), size = 5, colour = "purple") + 
  labs(x = "Species", y = "Bill length (mm)",
       title = "Bill length in Palmer Penguins") +
  theme_bw() 
...
![](images\images/patchwork.png){width=60%}
...
Assuming you have been creating the code for your figures in the notebooks, the plots appear within the notebook (rather than in the plot panel at the bottom right in RStudio). You can change the settings on the notebook to have the output appear in the console rather than within the notebook (click on the little gear symbol next to Preview, then tick Chunk Output in Console). New plots will appear in the plot panel and you can use the Export menu item to save the figure.


To export them in a format that will import nicely into Microsoft Word for your report, we suggest a `.png` file (you can also save to `pdf`, `jpeg`, and other formats).
...
It's good practice to save your plots into an `output/` folder. Create one in your prac folder by clicking "New Folder" and naming it "output".
...
You can also save the figure in other formats:
...
If you want your table output to print nicely in your notebook, we can enlist two extra packages: `kableExtra` and `parameters`. We'll use `parameters` to tidy our model output and `kableExtra` to decorate your table so it looks nice in the end. You will need to install `kableExtra` if you want to do the following.
...
In most cases, if you are writing your report in MS Word, you will want to export your table and edit it in MS Excel. We will use `write_csv` to do this. Save the table in the `output/` folder you created earlier.
...
For MS Excel, you can edit the style of your table more freely, adding borders or bolding text however you like! To put it in a MS Word document, simply highlight the parts you want and copy and paste them into your document.
...
Also useful resource to get started with is: Chang, W (2012) R Graphics Cookbook. O’Reilly Media—a guide to ggplot.
