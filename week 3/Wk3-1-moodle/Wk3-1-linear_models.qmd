...
We can use the `estimate_contrasts()` function to give us pairwise comparisons between all groups. Try it out
...
A relatively small value in our `F value` means that we have relatively more `r cloze("noise relative to signal", c("noise relative to signal", "signal relative to noise"))` in our data. 
