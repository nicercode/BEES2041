...
In a linear model with categorical variables, the slopes are always relative to the reference group in the `(Intercept)` or $\beta_0$ i.e. `temp_treatment [20C]`. In this example, The two addition predictor variables $x_1$ and $x_2$ are binary variables (a variable made one 1s and 0s). $x_1$ refers to whether a fish belonged to `temp_treatment [24C]` or not and $x_2$ indicates whether a fish belonged to `temp_treatment [26C]`. 
