...
# Data for prac
data_towers <- read_csv("week 3/Wk3-1-moodle/data/towers-2024.csv")
data_martino <- read_csv("week 3/Wk3-1-moodle/data/martino-fish-smr.csv")
...
![Meet Dr. Isaac Towers! Image credit: Data Dan](isaac-plant-ecosystems.png){60%}
...
# Jasmine, fish and energetics in a warming world
...
![Introducing marine biologist - Dr. Jasmine Martino!](jasmine-fish.png)
...
### Linear model with categorical predictor(s)
...
![Using a linear model with categorial and continuous variables](lm-anova.png)
```{r}
...
![Under the hood, maths behind an ANOVA](signal-to-noise.png)
...
For both the predictor (`temp_treatment`) and `Residuals` we do some math shown (but we use R instead) to calculate: 
...
### Estimate mean differences of each group
...
### Slopes when x is a categorical variable
...
![Categorical variables becomes multiple predictors in a linear model](lm-multiple-predictors.png)
Importantly: 
...
> How does the linear model create slopes for mean differences? 

### Encoding groups in a linear model

![Encoding categorical data as binary (0/1) variables](contrasts.png)
In a linear model with categorical variables, the slopes are always relative to the reference group in the `(Intercept)` or $\beta_0$ i.e. `temp_treatment [20C]`. In this example, The two addition predictor variables $x_1$ and $x_2$ are binary variables (a variable made one 1s and 0s). $x_1$ refers to whether a fish belonged to `temp_treatment [24C]` or not and $x_2$ indicates whether a fish belonged to `temp_treatment [28C]`. 

> Okay, each slope is relative to the intercept `temp_treatment [20C]`, what if I want to compare between `temp_treatment [24C]` and `temp_treatment [28C]`

### Estimate pairwise comparisons of group means (contrasts)
...
<!-- TODO: -->
<!-- - Fitting a lm to a categorical variable is actually a linear regression with multiple predictors -->
<!-- - Intro to ANOVA -->
<!--   - group slopes + contrasts -->
<!--   - paramter estimates test -->
<!--   - anova test -->
<!--   - pairwise comparisons -->



# Island differences in Palmer Penguins

![Location, location, location. Image Credit: [Mario Gregorio Saavedra Rodríguez](https://rpubs.com/mgsaavedraro/665527)](palmer-islands.png)
...
group_constracts_island <- estimate_contrasts(fit_penguins, contrast = "island")
...
ggplot(group_means_spray, aes(x = spray, y = Mean)) +
  geom_jitter(data = InsectSprays, aes(x = spray, y = count), color = "red", width = 0.1) + # Raw data
