...
### P-values - how likely is it to get our results by chance?

> The p-value is derived from the F-distribution

![](f-value-p-value.png)
The p-value and the F-statistic are closely related because the F-statistic is used to test whether the model explains a significant portion of variance, and the p-value tells us the probability of observing such an F-statistic under the null hypothesis.

The p-value tells us how extreme the observed F-statistic is under the null hypothesis (which assumes the predictor has no effect).
...
```{r}
means <- estimate_means(towers_fit, by = "log10_rainfall")
means
```
