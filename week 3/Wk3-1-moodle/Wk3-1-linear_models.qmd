...
# Clean, nice summary
parameters(penguins_fit) confint(penguins_fit)
...
Once you've done the above, use your outputs to answer the following questions

**Questions: **

1. What does the slope tell us? 

- There is a postive/negative relationship between bill length and body mass

2. Try interpretting the slope for us

- A unit increase in body_mass_g corresponds to a 2.690e+01/4.051e-03/<2e-16 in bill length

3. Yikes, looks like by rounding, the confidence intervals for the slope in the output of `parameters()` is not very helpful. Take a look at `report()` instead OR you can try `confint(penguins_fit)` to look at the unrounded values of the confidence intervals

- What does the confidence intervals for the slope tell us? 

The confidence intervals for the slope tells us that the true population mean for slope lies between
24.402502194 and 29.395242653 / 0.003467795 and 0.004635038

4. Let's get some meaning from the R^2 and F-statistic from your model. What are they telling us?

The R^2 value suggests that there is a weak/moderate/strong relationship between bill length and body mass. 

The F-statistic is relatively small/large, meaning that bill length/bill depth/flipper length 

5. How likely is it to observe our result? Do we have evidence 

# Our recovering Ozone

![Different layer's of the Earth's atmosphere, Image credit: [ABC News](https://www.abc.net.au/btn/classroom/ozone-recovery/101917894)](ozone.png)
