...
Linear models assume **constant variance**. We learned to transform data and which can help us fit better linear models but in some cases, transformations is not enough to fulfill the assumptions of a linear model. 
...
In situations where our response is discrete and exhibit a mean-variance relationship, we turn to **GLMs**! 
...
It may help to visualise the **link functions**. Let's consider a **binomial** response, where the mean of the response variable is given by the **logit** function above.  Plotting the function shows the relationship between $x$  and the mean of the response variable.
...
Now let's consider a **Poisson** response. The link function $g$ is the **log** function, such that
...
Now imagine drawing random numbers, where the **mean rate of events occurring** is given by $\mu_y$. In any one period, you could have fewer, or more than $\mu_y$ by chance. But overall, the number of events will follow $\mu_y$. 
...
![Feeling crabby with R?](images/crabs.jpg){width=60%}
...
Lets try working with some count data collected by some students our department who had surveyed a beach. This group of students wanted to analyse **how the presence of a crab varied with time and distance from the water line**. 
...
Let's try visualise this raw data. Here I am making violin plots and added the counts with `geom_jitter`. I also used `coord_flip` to flip the axes so its easier to look at the distribution of the data
...
Let's try a GLM usng `family = "poisson"` because we are working with counts of terrestrial slugs.
...
We use `estimate_means()` to calculate the mean count of terrestrial slugs for each treatment and use this to create a graph.
...
- Install and load the `MASS` package and the bacteria dataset. 
...
The p-value is `r cloze("0.011", c("0.011", "0.1", "11"))`. This indicates there is relatively `r cloze("low", c("low", "high"))` that there is no relationship between the presence of bacteria _H. influenzae_ and weeks.
...
2. **Interpret the slope for `Elevation`**
...
The CI for the slope is  `r cloze("-0.00410", c("1.55", "-0.00410", "0.110"))` (lower) and `r cloze("-0.00157", c("-0.00157", "1.98", "0.95"))` (upper). The interval represents where the true `r cloze("population", c("sample", "population"))` mean estimate for the `r cloze("slope", c("slope", "intercept", "counts"))` could lie 95% percent of the time. 
...
## Binomial: Varying the intercept of the model
...
## Poisson: Varying the intercept of the model
