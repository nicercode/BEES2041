...
Before thinking about GLMs, let's refresh our understanding of probability distributions.

We assume that the response variable follows a probability distribution. The choice of distribution depends on the nature of the response variable.

For example, 

- if the response variable is continuous, we might use a normal distribution.
- If the response variable is binary (e.g., presence/absence), we might use a binomial distribution.
- If the response variable is a count (e.g., number of species), we might use a Poisson distribution.



## Binomial distribution

The binomial distribution is used to model the number of successes in a fixed number of trials. The distribution has one main parameter: the probability of success $p$. It generates a series of 0s and 1s, where 1 represents a success.

For exmaple, we can simulate observations trials with a probability of success of 0.9:
...
The Poisson distribution is used to model a count of events per unit of sampling (usually time, space or volume). The distribution has one parameter $\lambda$, which is the mean number of events per unit of sampling.

It generates a series of integers, starting at 0, representing the number of events.

For exmaple, we can generate the number of events in 10 sampling units with a mean rate of 1.5 events per unit, 
...
It maky help to visualise the link functions. Let's consider a binomial response, where the mean of the response variable is given by the logistic function above.  Plotting the function shows the relationship between $x$  and the mean of the response variable.
...
p3 <-
  ...
  geom_line(data = data, aes(x = x, y = mu), color = "red", linetype = "dashed")
...
```{r}
# Fit the model
model <- glm(y ~ x, data = data_y, family = "bionmial") 
