...
For each additional unit of distance, the odds of crab presence increase by a factor of exp(0.05804) ≈ 1.06, though this effect is not statistically significant (p = 0.364).

## Visualising our model estimates

When we get a log-odds (logit) coefficient from logistic regression, it’s not directly interpretable as a probability. We need to backtransform it using the logistic function:
...
The default link function for Poisson is log(). If we write the mean count as $λ$

$log(λ)=−0.92+2.12×Treatment$
...
The histogram shows that the counts of `Faramea.occidentalis` are right-skewed/left skewed. The dataset consists of mostly 0s/10s/20s. This is considered count/binary/continuous data and would be best represented by a binomial/normal/Poisson distribution. 

2. Interpret the slope for `Elevation`

The slope is negative/positive, indicating the counts for _Faramea occidentalis_ decreases/increases with increasing elevation/temperature/density.

3. Interpret the CI for `Elevation`. Hint: you may need to pipe `parameters(faramea_fit)` into  `tibble()`

The CI for the slope is -0.00410 and -0.00157. The interval represents where the true population mean estimate for the slope/intercept/counts could lie 95% percent of the time. 
...
The distribution for `Galumna` is left-skewed, meaning there are lots of small/large values. This data is count data and resembles a binomial/Poisson/normal distribution.

2. Make a scatterplot of `Galumna` and `WatrCont.` Why do you think the data is not suited for a linear model? 

The scatterplot shows that the data is not continuous/random/corrected (there are horizontal lines of points). The plot also shows that the points are not evenly spread out/outliers. These characteristics suggests that a linear model is not suited for this type of data.  

3. Interpret the slope `WatrCont` and its p-value. 

The slope for WaterCont is negative, suggesting that as water content increases/decreases, the counts of Galumna decreases. The p-value tells us that there is very low probability that there is no relationship between water content and mite count.
...
The mean/maximum/minimum counts for Galumna, starts to taper off at 600/400/200.
