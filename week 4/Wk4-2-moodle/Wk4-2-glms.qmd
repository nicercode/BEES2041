...
<!-- A GLM makes some important assumptions that we will check shortly:  -->

<!-- 1. The observed y are independent, depends on some predictors x -->

<!-- - We can’t check this assumption from the model results, but you can ensure it’s true by taking a random sample for your experimental design.  -->

<!-- 2. The response y come from a known distribution with a known mean-variance relationship -->

<!-- 3. There is a straight line relationship between a known function $g$ of the mean of $y$ and the predictors  -->
<!-- $x$ -->


<!-- Note: R may prompt you to install some additional packages when checking a GLM model -->

```{r}
check_model(fit_crab)
```

## Look at model output

If all the assumption checks are okay, we can have a look at the results the model gave us. The two main functions for inference are the same as for linear models: `summary()`, `paramaters()` and `anova()`.
