...
```{r}
# The link that contains our data
...
Generally speaking, model checks are pretty standard and not very exciting. 
...
There are two ways we can improve the model fit (i.e. satisfy the assumptions) for a linear model: 

- apply a transformation on the raw data (on the x and/or y variables)
  - log, typically, log 10 `log10(var)` 
  - square root `sqrt(var)`
- if any, remove outliers 

Here we will apply a `log10` transformation to the two relevant variables `leaf_mass_per_area` and `rainfall`. (We apply `log10` because this often helps when the data has a right-skewed distribution.) We will be creating a new variable using `mutate()` and will be assigning these new changes to `data_towers` into a new data object called `data_towers_transformed`.
...
The file `data/GrowthRate.csv` contains 138 observations on the relationship between individual growth rate `GrowthRate` of a wide range of organisms and average individual weight `BodyWt`. The dataset contains species whose size varies over 10 orders of magnitude from approximately 10^-4 g to 621 kg.
...
ggplot(data_growthrate, aes(x = BodyWt, y = GrowthRate)) +
  geom_point() # Data is widespread and right-skewed and should be log10 transformed
...
`r cloze(c("high variance in larger values", "x and y may not follow a linear relationship", "there may be some outliers, but hard to tell because the data has a massive spread", "the residuals are non-normal"), possible_assumption_issues)`
...
3. Humans are considered to be a big brained species. Is there evidence from the analysis to suggest that humans are an unusual species relative to the others? Could we consider it as an 'outlier'? You may need to create a plot and use some clever labelling of points to answer this question.
...
The data we will explore here are a subset of Hooker's data containing 31 observations:
- `Temperature` at boiling point of water (degrees Fahrenheit)
- `Pressure`, the corrected barometric pressure (inches of mercury). 

> We want to know whether changes in pressure (which decreases with altitude) affect the boiling point of water. 
...
# Check model
check_model(hooker_norow7_fit) # Not really improving things..
...
data_hooker_transformed |> 
  mutate(obs_id = row_number()) |> 
  ggplot(aes(x = log10_pressure, y = log10_temp)) + 
  geom_text(aes(label = obs_id)) + # A scatterplot where each point is the observation number
  theme_classic()
...
data_hooker_transformed_norow7 |> 
  mutate(obs_id = row_number()) |> 
  ggplot(aes(x = log10_pressure, y = log10_temp)) + 
  geom_text(aes(label = obs_id)) + # A scatterplot where each point is the observation number
  theme_classic()
...
**Questions**
1. When fitting a linear model on the raw data, which assumptions are violated? Select all that apply.
...
`r cloze(c("there are potential outliers", "non-linear relationship present", "non-constant residual variance", "residuals are non-normal"), possible_violations)`
...
Load the data file `data/Amphipod_fecundity.csv` and choose a plot to visualise egg number per brood, you may want to label each point as the row number again.
...
parameters(amphipods_fit)
summary(amphipods_fit)
...
For every 1 mm increase in body length in an amphipod, we see an average `r cloze("increase", c("increase", "decrease"))` of `r cloze("5.64", c("5.64", "31.02"))` eggs. 

1. How many eggs would you predict that a 10 mm female would have?

The fitted equation of our model is:

`r cloze("number of eggs (fecundity)", c("number of eggs (fecundity)", "body length"))` = `r cloze("-25.13", c("6.34", "-25.13"))` + `r cloze("5.64", c("5.64", "-31.02"))` * `r cloze("body length", c("number of eggs (fecundity)", "body length"))`

Substituting 10mm in place of `r cloze("body length", c("number of eggs (fecundity)", "body length"))` in the fitted equation will give us `r cloze("31.27", c("31.27", "-1966.668", "-94.42"))` eggs.

5. What percentage of the variance in egg number per brood is explained by your relationship?

The `r cloze("R-squared", c("p-value", "t-value", "F-statistic", "R-squared"))` value tells us our model explains `r cloze("76%", c("24%", "58%", "76%"))` of the variance.

If you noticed an outlier and removed it, you would've gotten slightly different statistics to the answers. The outlier deserves closer inspection. You can exclude the point if you find that there is a valid reason to exclude it, for example, it is physically or biologically implausible. If not, you should keep it but ask the extent to which your conclusions are dependent on the inclusion or removal of the point. If your conclusions are very dependent on it, you might have to check the point (data or field notes), or even go back out and get more samples around that area. If you do re-measure and find that the measurement is real, you should obviously keep it.

## Example 5: River flow and density of larval fish

![](images/retropinna-semoni-cropped.jpg) <br>

River managers want to know if there is a difference in the numbers of fish recruiting under different environmental flow practices. They go out to a number of different locations along a river of interest and measure the density of larval fishes and water flow rate (velocity m/s) at each site.
...
Load the data file `data/koalas.csv` and choose a plot to visualise the number of koalas observed `Koalas` and year of the last fire `Year.last.fire`.
...
For every year since a bushfire, the number of koalas `r cloze("increased", c("increased", "decreased"))` by 0.21. 
