...
The logit function is also called the log-odds function, because it is the logarithm of the odds $\frac{p}/{1-p}$. In a standard linear regression ($y = \beta x$), the units of $\beta$ are "units of $y$ per unit of $x$." In a binomial GLM, the "units" of the left-hand side are log-odds.

$$\underbrace{\ln\left(\frac{p}{1-p}\right)}_{\text{Unit: Log-odds}} = \underbrace{\beta_0 + \beta_1x_1}_{\text{Unit: Log-odds}}$$

Because the math on the right-hand side (RHS) is strictly linear, the coefficients ($\beta$) must represent the change in the log-odds for a one-unit change in $x$.
