...
- **Binomial** data -- useful for modeling **probabilities**, like whether an event happens or not (e.g., predicting invasive species presence/absence, or organism survival).
- **Count** data -- great for modelling things that happen a certain number of times, like the number of visits to a nesting site or the number of species in a sample.
- **Count** data -- great for modelling things that happen a certain number of times, like the number of visits to a nesting site or the number of species in a sample.
...
We learned to transform data, which can help us fit a better linear model, but in many cases this is not enough to fulfill the assumptions of a linear model. 

In these situations,  a **G**eneralized **L**inear **M**odel (**GLM**) is often a good tool. 

**Probability distributions**

GLMs are a generalisation of linear models that allow us to model different types of responses. As data scientists, we need to choose an appropriate distribution that represents our data. For example: 
...
For example, we can draw 10 numbers (**trials**) from the **binomial distribution** with a **probability of success** of 0.9. You can change the `prob` value and see how that affects **outcome** of the trials
...
So our mean response is obtained via a transformation of $x$, creating a model that has a non-linear response, but is still based on a linear model in $x$. 
...
The plot below shows data which were drawn from such a model. The black dashed line shows the true relationship between the linear predictor $x$ and the mean of the response variable. The dots show the observations. 
...
You might also notice that the column name for the parameter estimates is 'Log-Odds', instead of 'Coefficient' like with linear models. 

The logit function is also called the log-odds function, because it is the logarithm of the odds $\frac{p}{1-p}$. In a standard linear regression ($y = \beta x$), the units of the slope $\beta_1$ are "units of $y$ per unit of $x$." In a binomial GLM, the "units" of the left-hand side are log-odds, so therefore the slope coefficient must represent the change in the log-odds for a one-unit change in $x$.

$$\underbrace{\ln\left(\frac{p}{1-p}\right)}_{\text{Unit: Log-odds}} = \beta_0 + \beta_1x_1$$

## Worked Example: Crabs along the beach

Let's apply the theory we learned about with some data collected by some students in our department who had surveyed a beach. This group of students wanted to analyse **how the presence of a crab varied with time and distance from the water line**. 
...
It's always good practice to take a look at your raw data, especially your response and explanatory/predictor variable.
...
ggplot(crab_means, aes(x = Dist, y = Probability)) +
  ...
  geom_line(linewidth = 1, col = "darkorange") +
...
- Fit an appropriate model for the presence/absence of Galumna sp. `pa` as a function of substrate density `SubsDens` as follows, using the `glm()` function and the `family` argument
- You may need to make some plots to help you answer the following questions
- Use your model output to help you answer the questions
...
`pa` is a `r cloze("binary", c("binary", "continuous", "random"))` variable, where 1 indicates presence of mites (conventionally, 1 indicates presence). We need a GLM to model this data, because this data is not `r cloze("continuous", c("binary", "continuous", "random"))` and comes from a `r cloze("non-normal", c("normal", "non-normal"))` distribution and cannot be represented by a linear model. 

1. **What does the Intercept represent in this model?**

The Intercept in this context represents the log-odds when substrate density is `r cloze("0", c("1", "0", "100"))`.
...
The p-value is `r cloze("< 0.001", c("0.360", "< 0.001", "< 0.0001"))` for the slope `SubDens`. This indicates that there is a `r cloze("low", c("moderate", "low", "high"))` probability that there is `r cloze("no", c("a strong", "no"))` relationship between the presence of mites and substrate density. This means we have strong evidence that substrate density affects the presence of mites.

## Example 2: Bacterial treatment 

![](images/bacteria.jpeg){width=60%}

Using the bacteria dataset (from the `MASS` R package), try and model the presence of _H. influenzae_ as a function of the week of testing. 
...
We are interested in how presence/absence of _H. influenzae_ changes across weeks.

**Exercises:**

- Load the bacteria dataset
- You will need to convert `ap` into a binary format
- Fit an appropriate model for this type of data
...
# Convert `ap` to binary format
bacteria <- bacteria |> 
  mutate(y = if_else(ap == "p", 1, 0))
...
```{r solution2}
# generate data
# MASS::bacteria |> select(-ap) |> mutate(ap = if_else(y == "y", "p", "a")) |> 
#   select(-y) |> 
#   write_csv("data/bacteria.csv")
...
`y` is the `r cloze("presence and absence", c("presence and absence", "count", "density"))` of the bacteria _H. influenzae_. A `r cloze("binomial", c("binomial", "Poisson", "normal"))` distribution will represent this type of data well. 
...
The p-value for the slope `week` represents the probability there is `r cloze("no", c("no", "a strong", "a weak"))` relationship between the `r cloze("presence of bacteria H. influenzae", c("presence of bacteria H. influenzae", "intercept"))` with `r cloze("week", c("week", "patient age", "patient location"))`.
...
In other words, the higher $x$, the more events are predicted to occur in a given period. 

## Response distribution

Now imagine drawing random numbers, where the average rate of events is given by $\mu_y$. In any one period, you could have fewer, or more than $\mu_y$ by chance. But overall, the number of events will follow $\mu_y$. 
...
Note the true values we used to simulate the data are $B_0 = 0$ and $B_1 = 0.8$. So the model has done a reasonable job of estimating the slope, and the true values are included in the 95% confidence interval.

Here, again, you might notice that the column name for the parameter estimates is 'Log-Mean', instead of 'Coefficient' like with linear models. 

In a standard linear regression ($y = \beta x$), the units of the slope $\beta_1$ are "units of $y$ per unit of $x$." In a Poisson GLM, the "units" of the left-hand side are log-mean, so therefore the slope coefficient must represent the change in the log-mean for a one-unit change in $x$.

$$\underbrace{\ln\left(\mu\right)}_{\text{Unit: Log-mean}} = \beta_0 + \beta_1x_1$$
...
Let's work with some data collected by some students in our department who wanted to investigate **how the abundance of slugs varied under two experimental treatments**. 
...
ggplot(data = data_revegetation, aes(x = Treatment, y = Soleolifera))  + 
  geom_violin(fill = NA) + 
  geom_jitter(aes(colour = Treatment), alpha = 0.5, width = 0.2, size = 2) +   
  theme_minimal() + 
  coord_flip()
```
...
The dataset contains the number of trees of the species _Faramea occidentalis_ measured in 43 quadrants in Barro Colorado Island in Panama. For each quadrant, environmental characteristics were also recorded such as elevation or precipitation. Let us take a look at the number of _Faramea occidentalis_ found in each quadrant.

We are interested in the effect of elevation on the number of trees of _Faramea occidentalis_.
...
The CI for the slope is  `r cloze("-0.00410", c("1.55", "-0.00410", "0.110"))` (lower) and `r cloze("-0.00157", c("-0.00157", "1.98", "0.95"))` (upper). It is likely that the true `r cloze("population", c("sample", "population"))` value of the `r cloze("slope", c("slope", "intercept", "counts"))` lies within this range.
...
The distribution for `Galumna` is `r cloze("right-skewed", c("right-skewed", "left-skewed"))`, meaning there are lots of `r cloze("small", c("small", "large"))` values in the variable. This data is count data and resembles a `r cloze("Poisson", c("binomial", "Poisson", "normal"))` distribution.
...
4. **Make a plot displaying the mean counts for Galumna at different `WatrCont`, at what level of WatrCont do we expect mite counts to go below 1?**

We can be confident that the `r cloze("mean", c("minium", "mean", "maximum"))` counts for Galumna likely go below 1 at a water content of approximately `r cloze("350", c("250", "350", "600"))`.
