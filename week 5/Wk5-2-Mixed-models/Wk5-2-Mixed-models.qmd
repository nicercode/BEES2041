...
rprojroot::has_file("BEES2041-code.Rproj") |>
  rprojroot::find_root() |>
  file.path("week 5/Wk5-2-Mixed-models") |>
  setwd()
...
In many real-world datasets, our observations are not entirely independent — measurements might be grouped by subjects, locations, time points, or experimental units. Ignoring this structure can lead to biased estimates and misleading results. If samples are taken in a similar group/area/time, we need to reflect this structure in our model. Otherwise we are not accurately reflecting the proper sampling structure. 

**Mixed models** help us handle this complexity by explicitly modeling both **fixed effects** — the overall trends we're interested in — and **random effects** — additional variation due to these structured sampling.
...
- **understand** the difference between random intercept and random slope
- **run** a linear mixed effects model in R using `glmmTMB()`
- **interpret** the output of your mixed effects model using `parameters()`, `estimate_expectation()`, `performance()`
- **understand** differences between marginal and conditional R^2 values.
- **make model predictions** using your mixed model using `estimate_expectation()`
...
> We will be using two new packages for today called `glmmTMB` and `remotes`. Uncomment the last two lines in the next code block to install it in your own devices.
...
> Remember to load the packages into R to use them!
...
At their core, random factors are reflect structure in the way data is sampled. Let's look at an example. The data we will analyse here are **counts of invertebrates at 3-4 pristine and modified sites in each of 7 (randomly chosen) estuaries**.

**Our research question is: How does total invertebrate counts differ between pristine and modified sites across all estuaries?**

Here the estuaries are the **random effect**, as there are **a large number of possible estuaries**, and we only sampled a few from the wider population of them, and we made multiple samples per estuary. We would like to make inference about estuaries in general, so add estuary as a random effect. This also tells the model that certain data points (from the same estuary) have something in common, so are not independent from one another, 
...
- `Modification` is our categorical predictor variable (x)
- `(1 | Estuary)` is a **random intercept**. This tells the model that we are allowing the intercept to vary by Estuary (mean `Total` at each `Estuary`).

> Technically `Total` is count data, but its distribution doesn't really resemble a Poisson distribution. A normal distribution fits it quite well so we can use a normal response here.
...
By now, you are familiar with `estimate_means()` and `estimate_contrasts()`. For mixed models, we need to use a different function `estimate_expectation()` to compute means for each level of our predictors and random effect.
...
There are a lot of repeated rows here! This is because `estimate_expectation()` generates mean predictions for the same number of rows and replicates we had in the data. You can verify this by counting the number of rows for `Modification` and `Estuary` and the values should match with our earlier table of counts.

```{r}
means_by_mod_estuaries |> 
  count(Modification, Estuary)

nrow(means_by_mod_estuaries)
```
...
Here we are plotting our raw data across different estuaries and faceting the plot by our variable of interest `Modification`.
...
Since this is a simple model with 1 fixed effect, we can use the `parameters()` output but if you prefer, you can also use `estimate_contrasts()` to see whether we have a significant difference between `Modified` and `Pristine` sites. The numbers are the same.
...
**2. Interpret the `Modification [Pristine]` coefficient and p-value**

The `Modification [Pristine]` coefficient is `r cloze("-14.52", c("-14.52", "40.97", "7.42"))`. This value represents the mean difference in `Total` number of invertebrates between `Modified` and `Pristine` sites. The value is `r cloze("negative", c("negative", "positive"))` which means the mean in `Total` is `r cloze("lower", c("higher", "lower"))` for `Pristine` sites.

## Your turn: Urchins

![**Urchin grazing**](images/Urchins.jpg){width=60%} <br>

In the last prac, we worked with this sea urchin dataset. This data was collected by marine researchers in BEES who were studying sea urchins at Bare Island near the opening of Botany Bay ([Wright et al. 2005](http://www.int-res.com/abstracts/meps/v298/p143-156/)). Urchins are voracious grazers and at high densities can remove entire kelp forests and leave bare areas known as urchin barrens. 
...
We fitted a linear model containing **one categorical and one continuous predictor**, this is also known as an **ANCOVA**: 
...
The researchers collected data using quadrats, with repeat visits to each one. They have 360 samples, but these 360 samples are not independent of each other because they were collected from 40 quadrats, with 9 samples per quadrat. 

Failing to tell the model that our data has this underlying structure is misrepresenting the data. By treating all 360 samples as independent, we are ignoring the fact that some of the variation in our data is due to inherent differences between quadrats. We also know our 40 quadrats are really a sample from a much wider population of quadrats and we want be able to make inferences about any quadrat in general.  Hence, we need to fit a mixed model that accounts for the random effect of `Quadrat`.
...
- Generate means for `Total` for `Day`, `Treatment` and `Quadrat` 
...
- setting the `Quadrat` variable as a factor/categorical variable as it is recorded as integers.
...
Now fit the model with the random effect of `Quadrat` on the intercept. 
...
# Compute means
means_by_day_treat <- estimate_expectation(fit_urchins_random, by = c("Day", "Treatment"))
means_urchins_all <- estimate_expectation(fit_urchins_random)

# Fit a line for each quadrat
p1 <-
  ggplot(means_urchins_all, aes(x = Day, y = Predicted, color = Treatment)) +
  geom_line(aes(group = Quadrat), alpha = 0.5, linewidth = 0.5) +
  labs(
    x = "Day",
    y = "log(BareRock)"
  ) +
...
# Add in the overall line
p1 +
  geom_line(data = means_by_day_treat, alpha = 1, linewidth = 2)
...
**1. What does it mean then if we were to ignore `Quadrat` and not include it as a random effect?**


Ignoring `Quadrat` would likely lead us to `r cloze("overestimate", c("overestimate", "underestimate"))` our sampling effort and may lead to `r cloze("biased", c("biased", "unbiased"))` estimates of the treatment effect. 

Ignoring `Quadrat` would lead to two main issues. The first is that we are treating all 360 samples as independent, when in reality they are not. This can lead to an overestimation of the degrees of freedom and an underestimation of the standard errors, which can result in inflated type I error rates (i.e., finding a significant effect when there is none). The second issue is that we are not accounting for the fact that some of the variation in our data is due to inherent differences between quadrats. This can lead to biased estimates of the treatment effect and can also affect our ability to  generalise our results to other quadrats or similar systems. By including `Quadrat` as a random effect, we can account for these sources of variation and obtain more accurate and reliable estimates of the treatment effect.

**2.Look at the updated output of your mixed model. After including the random effect of `Quadrat`, is the effect of `Treatment` still significant?**

The effect of `Treatment` is still significant after including the random effect of `Quadrat`. The p-value for the `Treatment [Cage control]` coefficient is `r cloze("0.001", c("0.001", "0.05", "0.1"))`, which is less than the commonly used significance level of 0.05, indicating that there is a statistically significant difference in `logBareRock` between the `Cage control` treatment and the reference level (which is likely the `Control` treatment).
...
In this example, we will use the [**Biomass And Allometry Database (BAAD)**](https://esajournals.onlinelibrary.wiley.com/doi/abs/10.1890/14-1889.1), which contains data on plant structure compiled by Data Dan, using studies from all around the world. You can find more information about the BAAD dataset [here](https://github.com/dfalster/BAAD). This dataset enables us to ask questions about how plant structure varies across species and environments, and how the dimensions of different plant parts (tree height, stem diameter, leaf area, biomass, etc.) are related to each other.
...
- estimate **how much the total leaf area of the tree increases with its height**, 
...
- however...there's a lot of **unexplained variation** in the data (above and below the line). So any given tree could likely have a leaf area that is quite different from the mean.
...
```{r}
# Estimate the predictions for the model for height x species
...
Another way to look it is that we've moved a bunch of variance from "unexplained" into variation between species in slope and intercept. 

We can assess whether the variation in the data due to the random effect is significant by comparing the mixed model to the simple model. We can do this using an `anova()` test , which will tell us if the random slopes model is significantly better than the simple model. It is!
...
There is a lot of interest in estimating the amount of carbon stored in forests, and using growing trees to remove carbon from the atmosphere. Since biomass is mostly (~42%) carbon, we can use the biomass of a tree as a proxy for the amount of carbon stored in the tree.
...
data_trees <- baad$data |>
  ...
  filter(!is.na(stem_diameter_m), !is.na(biomass_kg)) |> # Exclude NA in stem_diameter_m and biomass_kg
  mutate(
    log10_stem_diameter_m = log10(stem_diameter_m), # log10 transform 
    log10_height = log10(height), # log10 transform
    log10_biomass_kg = log10(biomass_kg) # log10 transform 
  )
...
```{r solution}
# Fit a model with random slopes for species and location
fit_stem <- glmmTMB(log10_biomass_kg ~ log10_stem_diameter_m + (1 + log10_stem_diameter_m | species), data = data_trees, REML = F)

# Estimate means
means_all <- estimate_expectation(fit_stem, by = c("log10_stem_diameter_m"))
means_sp <- estimate_expectation(fit_stem)

# Plot results
ggplot(data_trees, aes(x = log10_stem_diameter_m, y = log10_biomass_kg, color = species, group = species)) +
  geom_point(alpha = 0.3, show.legend = FALSE) +
  geom_line(data = means_sp, aes(y = Predicted), alpha = 0.5, linewidth = 1) +
  geom_line(data = means_all, aes(y = Predicted, group = 1), alpha = 1, linewidth = 2) +
  theme_minimal() +
  theme(legend.position = "none") 

# Look at model output
parameters(fit_stem)
performance(fit_stem)

## Estimate biomass of a tree with a stem diameter of 20cm
data_pred_stem <- tibble(
  log10_stem_diameter_m = log10(0.2))

# Make model prediction for a tree that is 10m tall and back transforming prediction
estimate_expectation(fit_stem, data = data_pred_stem) |> mutate(biomass_kg = 10^Predicted)

# Fit a simpler model to compare (optional but insightful)
fit_stem_simple <- glmmTMB(log10_biomass_kg ~ log10_stem_diameter_m, data = data_trees)

# Outputs of simple model
parameters(fit_stem_simple)
performance(fit_stem_simple)

estimate_expectation(fit_stem_simple, data = data_pred_stem)|> mutate(biomass_kg = 10^Predicted)
```

**Questions:**

**1. What is the rate at which `log10_biomass` increases with `log10_stem_diameter_m`?** `r cloze("2.49", c("1.03", "2.49", "3.36"))`

**2. What does the conditional R^2^ capture?**

the `r cloze("variation explained by the fixed and random effects", c("variation explained by the fixed effects", "variation explained by the random effects", "variation explained by the fixed and random effects"))`
