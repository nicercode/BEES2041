...
- **interpret** the output of your mixed effects model using `parameters()`, `estimate_expectations()`, `performance()`
...
Random factors are hard to get your head around, and are best explained with examples. The data we will analyse here are **counts of invertebrates at 3-4 pristine and modified sites in each of 7 (randomly chosen) estuaries**.
...
By now, you are familiar with `estimate_means()` and `estimate_contrasts()`. For mixed models, we need to use a different function `estimate_expectations()` to compute means for each level of our predictors and random effect. 
...
**1. Compare the output of `parameters(fit_urchins)` and the `parameters()` output of your mixed model. How did including the random effect of `Quadrat` change the model output?** 
