...
- `(1 | Estuary)` is a **random intercept**. This tells the model that we are fitting an intercept that is allowed to vary by Estuary (mean `Total` at each `Estuary`).

> Technically `Total` count data, but its distribution doesn't really resemble a Poisson distribution. A Normal distribution fits it quite well so we can use a normal response here.
