...
In this example, we will use the `baad` dataset, which contains data on plant structure compiled by Data Dan, using studies from all around the world. 

To access the data, we will use the `baad.data` R package. To install this, you will need to run the following code:
...
ggplot(data_trees, aes(x = log10_height_m, y = log10_area_leaf_m2)) +
  ...
  geom_ribbon(data = means_all, aes(y = Predicted, ymin = CI_low, ymax = CI_high), fill = "white") +
  theme_classic() + 
...
- the confidence intervals are **narrow**, so narrow that we can barely see them! This indicates that the model is confident in its predictions about the mean relationship.
- there's a lot of **unexplained variation** in the data (above and below the line). So any given tree is likely could have a leaf area that is quite different from the mean.
...
10 cm^2. This is a huge range! Surely we can do better!
...
Let's plot this mixed model fit and compare it to our first simple model.
...
- The grey line gives the average relationship across all species. This mixed model is able to capture the variation in the relationship between height and leaf area between species. 
