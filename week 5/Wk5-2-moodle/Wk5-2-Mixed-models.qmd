...
> We will be using a new package today called `glmmTMB`. Uncomment the last line in the next code block to install it in your own devices.
...
Importantly, we know that our sample of estuaries are likely to harbour different abundances of invertebrates. For example, some estuaries may be more productive, or climate may be more favourable to invertebrates than others. Ultimately, we know that different estuaries are likely to differ, on average in `Total` number of invertebrates. This random effect of `Estuary` is something we want to account for in our model. 
...
- `(1 | Estuary)` is a **random intercept**. This tells the model that we are fitting an intercpet that is allows to vary by Estuary.
...
Not perefcet, but good enough! Let's move on to the next step.
...
Since this is a simple model with 1 fixed effect, we can use the `parameters()` output but if you prefer, you can also use `estimate_contrasts()` to see whether we have a signficant difference between `Modified` and `Pristine` sites
...
Broadly speaking, the CI of the mixed models are `r cloze("smaller", c("smaller", "larger"))`. Corresponding, the p values are `r cloze("smaller", c("smaller", "larger"))`.

**2. What does it mean then if we were to ignore `Quadrat` and not include it as a random effect?**

Ignoring `Quadrat` means that we are not accounting for `r cloze("noise due to Quadrat in the data", c("normality", "non constant variance", "noise due to Quadrat in the data"))` 

# Random Slopes

## Worked Example: Tree height vs leaf area

In this example, we will look at the relationship between parts of the tree height and leaf area. We will use the `baad` dataset, which contains data on plant structure compiled by Data Dan, using studies from all around the world. 

To access the data, we will use the `baad.data` package. To install this, you will need to run the following code:
...
Inthe example, we will look at the relationship between tree height and leaf area. Data Dan has asked you to estimate how much the total leaf area of the tree increases with it's height. measuring leaf area is hard, but measuring height is relatively easy. Dan wants to report on how much more leaf area there will be if we let the forests in an urban area grow taller and older.

The data in the BAAD dataset contains information on the sizes of tree parts for a number of species at different locations around the world. The data comes from a number of studies. The following code reshape the data so that it is in the form of a tibble, with columns for the study name, species, location, height, and leaf area. We've also 
...
data_trees <- baad$data |>
  as_tibble() |>
  select(studyName, species, location, height_m = h.t, area_leaf_m2 = a.lf) |>
  filter(!is.na(height_m), !is.na(area_leaf_m2))

data_trees 
```

Your first thought is to fit a simple linear model to the data. This model assumes that the relationship between height and leaf area is the same for all species and locations.
...
However, the data is not well described by a simple linear model. let's try a double log transformation to see if that helps.

```{r}
data_trees <- data_trees |> mutate(log10_height_m = log10(height_m), log10_area_leaf_m2 = log10(area_leaf_m2))

fit_simple <- glmmTMB(log10_area_leaf_m2 ~ log10_height_m, data = data_trees)

check_model(fit_simple)
```

Not perefct, but much better and good enough!

Let's plot the data and the model fit.
```{r}
...
ggplot(data_trees, aes(x = log10_height_m, y = log10_area_leaf_m2)) +
  ...
  geom_ribbon(data = means_all, aes(y = Predicted, ymin = CI_low, ymax = CI_high), alpha = 0.2, fill = "orange", linewidth = 0) +
  geom_line(data = means_all, aes(y = Predicted), alpha = 1, linewidth = 2, col = "orange") +
  theme_minimal()
```
...
- the confidence intervals are narrow, indicating that the model is confident in its predictions about the mean.
- but there's a lot of unexplained variation in the data. So any given tree is likely could have a leaf area that is quite different from the mean.
...
10^c(0.2, 2.4)
```
 10 cm^2. This is a huge range!

Can we do better?

## Linear model with random effect by species

We could try to improve the model by allowing the relationship between height and leaf area to vary between species. Since the dataset contains lots of species, we can hypothesise that some of the variation in the relationship between height and leaf area is due to differences between species. We can account for this by fitting a model with a random intercept and slope for species.
...
Let's plot the new model fit and compare it to the simple model.
...
ggplot(data_trees, aes(x = log10_height_m, y = log10_area_leaf_m2, color = species, group = species)) +
  ...
  labs(
    x = "log10_height (m)",
    y = "log10_area_leaf (m^2)"
  )
```

Wow! The model with random slopes for species provides a much better fit to the data. Each coloured line gives the relationship between height and leaf area for a different species. The black line gives the average relationship across all species. The model is able to capture the variation in the relationship between height and leaf area between species. As we thoguht, the relationship between height and leaf area is not the same for all species and there is a lot of spread between species.

## Quantifying the variance components

Now that we have a richer model, we can find out how much of the variation in the data is due to the fixed effect, the random effects, and the residual noise. We can quantify these via the r2 of the model, with and without the random effects. In the following, 

- `R2 (marg.)` ( Marginal r2 ) is the proportion of the total variance that is explained by the fixed effects alone. 
- `R2 (cond.)` ( Conditional r2 ) is the proportion of the total variance that is explained by the fixed and random effects together.
...
- log10 height explains 78.8% of the variance in log10 leaf area, 
- Adding differences between species explain an additional 17%, brining the total explained variance to 96.1%
- This leaves only 3.9% of the variance unexplained.
...
The simple model gives a slope of 1.71, the random slopes model gives a slope of 1.79. Also, the confidence intervals for the simple model are much narrower than the random slopes model. And they don't contain the slope estimate from the random slopes model. 

**Predictions**
Let's use this mdoel to estimate the leaf area of a tree that is 10m tall.
...
The simple mdoel is predicting a higher value with very narrow confidence intervals. The random slopes model is predicting a lower value with much wider confidence intervals.
...
- captures more total variation in the data and shows use where this variation is coming from (differences between species),
- estimates a steeper slope for the relationship between height and leaf area,
- provides a more conservative estimate (wider CI) for the leaf area of a tree that is 10m tall in a randome tree, due to differences between species,
- once species is know, provides a tighter rletaionhsip between height and leaf area.

We can conclude that the simple model is overly confident in its predictions, indicated by the narrow confidence intervals. It doesn't know that species can differ a lot from each other. And it suggests less leaf area gained per unit height. Using the random slopes model gives us a much more nuanced view on the data.

## Your turn: Stem diameter vs biomass

Another potential use of data from the BAAD is to qunatify the relationship between stem diameter and biomass. There is a lot of ineterest in estimate the amount of carbon stored in forests, and using growing trees to remove carbon from the atmosphere. Since biomass is mostly (~42%) carbon, we can use the biomass of a tree as a proxy for the amount of carbon stored in the tree.
...
data_trees <- baad$data |>
  as_tibble() |>
  select(studyName, species, location, stem_diameter_m = d.ba, height = h.t, biomass_kg = m.so) |>
  filter(!is.na(stem_diameter_m), !is.na(biomass_kg)) |>
  mutate(
    log10_stem_diameter_m = log10(stem_diameter_m),
    log10_height = log10(height), log10_biomass_kg = log10(biomass_kg)
  )

data_trees
```

Now give it a go! Unlike the example above, you don't need to fit the simple model first. Just go straight to fitting the mixed model with random slopes model. 

```{r solution}

# fit a model with random slopes for species and location
fit <- glmmTMB(log10_biomass_kg ~ log10_stem_diameter_m + (log10_stem_diameter_m | species), data = data_trees, REML = F)

# Plot results
means_all <- estimate_expectation(fit, by = c("log10_stem_diameter_m"))
means_sp <- estimate_expectation(fit)
...
fit_simple <- glmmTMB(log10_biomass_kg ~ log10_stem_diameter_m, data = data_trees)
parameters(fit_simple)
performance(fit_simple)

## Estimate biomass of a tree with a stem diameter of 20cm
data_pred <- tibble(
  log10_stem_diameter_m = log10(0.2))

estimate_expectation(fit, data = data_pred) |> mutate(biomass_kg = 10^Predicted)
estimate_expectation(fit_simple, data = data_pred)|> mutate(biomass_kg = 10^Predicted)
```

Questions:

1. What is the rate at which log10_biomass increases with log10_stem_diameter_m? 
2. What is the log10_biomass of a tree with a stem diameter of 20cm?
