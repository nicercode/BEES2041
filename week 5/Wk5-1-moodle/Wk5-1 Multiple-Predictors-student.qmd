...
The Mean Sq / F value for `sex` are ??? than the Mean Sq / F value for `species`. This means that ???/species has a weaker effect on body mass.
...
The coefficient for `sex [male] × species [Gentoo]` is ???. This represents the ??? mean body mass between female ??? penguins and male ??? penguins.

**4. Interpret the p-value for `species [Gentoo]`**

The p-value for `species [Gentoo]` is ???. This indicates that the probability for finding the difference of ??? in mean body mass between ??? Gentoo penguins and female Adelie pengions is very ???.
...
If we did not account for bill length as a covariate, we would conclude that there are only differences in bill depth between ??? and `r `cloze("Chinstrap", c("Adelie", "Chinstrap"))`. 
