...
`r cloze(c("sex", "species", "sex:species"), ("sex", "species", "sex:species"))`
...
1. What is the estimated effect (slope) of `log10_rainfall` on `log10_leaf_mass_per_area`? `r cloze("-0.54", c("-0.42", "close to zero", "3.81"))`
...
fit_log_urchin <- lm(logBareRock ~ Day + Treatment + Day:Treatment, data = data_urchins)

# Check model
check_model(fit_log_urchin)
```

```{r}
anova(fit_log_urchin)
```

```{r}
means_urchins <- estimate_means(fit_log_urchin, by = c("Day", "Treatment"))
