...
Today, we will go through four different use cases that have different combinations of predictor variables: 

<!-- #TODO: @Daniel, do we want to split 1. into 2 (Main and interactive effects?) -->

1. Two **categorical** predictors 
    - main additive effects
    - interactive effects 
2. Two **continuous** predictors (main effects)
3. Two **continuous** predictors (with interaction)
4. One **categorical** predictor, one continuous predictor
...
This means that we can fit a **two-way ANOVA** using the `lm()` function and then call `anova()` on the model fit object. 

We can also look at the output of using `parameters()` to see **HOW big the main/interactive effect** is. 

## Worked Example: Penguins: size by sex and species
```{r}
data2 <- data_penguins |>
  filter(!is.na(sex))
