...
- The `log10_rainfall` coefficient is the change in `log10_leaf_mass_per_area` for a one unit increase in log10 rainfall, holding temperature constant. As we found previously, **this effect is signifcant**, with rainfall having a negative effect on `log10_leaf_mass_per_area` by 0.54 units per unit of `log10_rainfall`.
...
```{r}
# Estimate the means
means_leaves_temp <- estimate_means(fit, by = c("log10_rainfall", "temperature"))

# estimate the overall means for rainfall only
means_leaves_overall <- estimate_means(fit, by = c("log10_rainfall"))
...
```{r}
estimate_slopes(fit_leaves_main, trend = "log10_rainfall")
```
