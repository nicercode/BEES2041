...
# Use case 3: One **categorical** and one **continuous** variable (ANCOVA) 
...
fit_urchin <- lm(BareRock ~ Day + Treatment + Day:Treatment, data = data_urchins)

# Check model
check_model(fit_urchin)
```

```{r}
# The response needs a log transoform
...
```{r}
p1 <-
  ggplot(means_urchins, aes(x = Day, y = Mean, color = Treatment)) +
  geom_line() +
  labs(
    x = "Day",
    y = "log(BareRock)"
