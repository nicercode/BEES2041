---
title: "Week 5-1 Multiple predictors"
output: html_output
editor_options:
...
rprojroot::has_file("BEES2041-code.Rproj") |>
  rprojroot::find_root() |>
  file.path("week 5/Wk5-1-Multiple-Predictors") |>
  setwd()
...
Some research designs are interested in the effects of **two categorical variables** on the continuous response variable. For example, supplement of vitamin C and exercise on the outcome of managing stress, or age group and diet in maternal reproductive success. In these instances, we use **Two-way ANOVA** (Analysis of Variance) - a statistical method that helps us to examine:

- **Main effects** – The **independent impact** of each categorical variable on the outcome.

- **Interaction effects** – Whether the **effect of one variable depends on the level of the other**.

> Recall that ANOVA at their core, are linear models. 

This means that we can fit a **two-way ANOVA** using the `lm()` function.

Using `anova()` on the model fit object, we can **test whether the overall main effect and interaction effect of the predictors are important** or not.
...
The **continuous response** variable (y) is: `body_mass_g`
The **two categorical predictor** variables (x1, x2) are: `sex` and `species`

First, let's exclude the penguins where `sex` is NA. These are juvenile penguins that won't contribute to answering our question.
...
- `sex:species` means we are interested in the **interactive effects** of both sex and species. In other words, the effect on body mass may depend on sex AND species.
...
Let's proceed with our two-way ANOVA to test whether sex and species and their interaction contributes to variation in body mass.
...
We can see our predictor variables listed as rows. **Each row tests whether each predictor significantly affects body mass**. 

- The first row for the main effect of `sex`
- The second row for the main effect of `species` after accounting for the main effect of `sex`
- The third row for the interactive effect of `sex` and `species` after the two main effects.

This output tells us that both the **main effects** of sex and species and their **interactive effect** have a strong effect on body mass variation in Palmer penguins. 

**Questions:**

**1. Which of the three effects are contributing significantly to variation on body mass? (Select all that applies.)**
...
Typically when we are working with **categorical** predictors, we look at the ANOVA output first to determine whether there is an effect of a predictor before looking at the model output using `parameters(fit_penguin_adults)`. 

The output of `parameters` gives us a snapshot of how big the effect of sex and species is for a reference group (female Adelie), but this is not as useful as `anova`. 
...
**2. What does `sex [male]` represent?**

The coefficient for `sex [male]` is `r cloze("674.66", c("674.66", "-262.89", "1310.91"))`. This represents the `r cloze("difference in", c("difference in", "p value for"))` mean body mass between female `r cloze("Adelie", c("Adelie", "Chinstrap", "Gentoo"))` penguins and male `r cloze("Adelie", c("Adelie", "Chinstrap", "Gentoo"))` penguins.

**3. Interpret the p-value for `species [Gentoo]`**

The p-value for `species [Gentoo]` is `r cloze("< .001", c("< .001", "0.014", "0.089"))`. This indicates that the probability for finding the difference of `r cloze("1310.91", c("1310.91", "3368.84", "674.66"))` in mean body mass between Gentoo penguins and female Adelie penguins is very `r cloze("low", c("low", "high"))` under the expectation of finding no difference (the null hypothesis).

## Means

After establishing that there are main and interactive effects on body mass in our ANOVA, we may want to use the model to get some **estimates of means** for different groups of sex, species, or both using `estimate_means()`.

What's useful is that you can estimate means for a main effect while holding the other effects constant. 

For example, here we are calculating the means of the two sexes and marginalising out the effect of species, i.e. taking an average with respect to sex, while controlling for species.

```{r}
estimate_means(fit_penguin_adults, by = c("sex"))
```

We can also calculate the means for all combinations of sex and species.

```{r}
estimate_means(fit_penguin_adults, by = c("sex", "species"))
```

**4. What does the 95% CI represent for `female | Gentoo` penguins?** 
...
We may also want to use the model to get some **estimates of differences in means** between sex and species using `estimate_contrasts()`.

What's useful is that you can estimate contrasts for a main effect while holding the other effects constant. 

For example, here we are calculating the difference in means between the sexes, taking an average over species.

```{r}
estimate_contrasts(fit_penguin_adults, contrast = c("sex"))
```

Or we can calculate the difference between the species, taking an average over sexes.
...
For female penguins, the difference between Chinstrap and Adelie was `r cloze("significant", c("significant", "non-significant"))`, while for male penguins, the difference between Chinstrap and Adelie was `r cloze("non-significant", c("significant", "non-significant"))`. This shows the value of having the contrasts between species for each sex, as when not considering sex, the model indicates that the overall difference between Chinstrap and Adelie penguins was `r cloze("non-significant", c("significant", "non-significant"))`, losing the ability to detect that the significance of the contrast varies between sexes.
...
Overall, our multiple linear regression model explained R^2 = `r cloze("85%", c("28%", "64%", "85%"))` of variation in body mass.  _Hint: look at the output of `summary(fit)`_

_Contrasts_

Generally female penguins are smaller than male penguins (mean difference = 630.51, 95% CI = 560.27 - 700.75, p-value = <0.001). 

There were `r cloze("strong", c("no", "weak", "strong"))` statistical differences in the body mass of male and female penguins for all species (here, you would reference the ANOVA table). 
...
There were `r cloze("no", c("no", "weak", "strong"))` statistical differences in the body mass of male Chinstrap and male Adelie penguins (Chinstrap-Adelie = -104.52g, 95% CI = [-230.90, 21.85], p-value =  0.105). There were `r cloze("marginal", c("weak", "marginal", "strong"))` differences in body mass between female Chinstrap and female Adelie penguins (Chinstrap-Adelie = 158.37, 95% CI = [31.99, 284.7], p-value = 0.014). All other pairwise combinations were highly significant. See Figure 1 above. 
...
A researcher is testing the effect of metal contamination on the number of species of sessile marine invertebrates (sponges, bryozoans and sea squirts etc.) but they also know that the richness of invertebrates can depend on whether the substrate is vertical or horizontal.

**The research question is: How much does copper and substrate orientation affect species richness?**
...
estimate_contrasts(fit_marine, contrast = "Copper", by = "Orientation")
estimate_contrasts(fit_marine, contrast = "Orientation", by = "Copper")
```

**Questions:**

**1. Which effect/s contributed to variation in species richness?**

`r cloze(c("Copper", "Orientation", "Copper:Orientation"), c("Copper", "Orientation", "Copper:Orientation"))` contributed to variation in species richness. This is reflected in the values of Mean sq, F value, and p value.
...
Additionally, Isaac asked **whether temperature also had an effect on leaf mass per area**. He first wanted to test the **main (additive) effects** of rainfall and temperature on leaf mass per area.
...
As we've done previously, we can use `estimate_means()` to get the means of y with respect to x1 and x2. Specifically, we can estimate the mean `log10_leaf_mass_per_area` for
...
ggplot(data_leaves, aes(x = log10_rainfall, y = log10_leaf_mass_per_area)) +
  ...
  geom_line(data = means_leaves_overall, aes(y = Mean), col = "orange", linewidth = 1.5) + # overall mean
  labs(
    x = "Log 10 Rainfall",
    y = "Log 10 Leaf mass per area"
  ) +
  theme_classic()
```

## Overall effect of rainfall in the second model
...
You can see the effect estimated here is very close to the number we calculated above in the additive model (-0.54). This makes sense.
...
**3. Based on these results, what can we say about the effect of `temperature` on `log10_leaf_mass_per_area`?**

`r cloze("It has limited direct effect, but changes the effect of rainfall on leaves", c("It is positive", "It has a strong effect", "It has limited direct effect, but changes the effect of rainfall on leaves"))`
...
These models can get as complex as you like, but we will look at the simplest version of these - let's look at the simplest scenario: a model with **one categorical and one continuous predictor**.
...
Read this data in and save it as an object. We're also going to log transform the y variable (`BareRock`) as it is a percentage and will be right skewed. (If you like, you can check this by fitting a simple model and checking the fit.)
...
We expect the % cover of bare rock to change over time, but we also expect different treatments to have different effects on the % cover of bare rock. Hence, we are really interested in whether the **slope of the relationship between `Day` and `logBareRock` changes with `Treatment`!**
...
Because we have a categorical effect, the `Intercept` is the mean `logBareRock` for the reference group. This is the first level of `Treatment`, which is `cage.control`, when Day = 0. Effects are then the difference for each predictor, relative to this reference group.

I wonder if there is a better way to look at the results?

## Visualising the results

By now you should be familiar with the workflow for visualising the results of a linear model. We use `estimate_means()` to get the means of `logBareRock` for each level of `Treatment` at different levels of `Day`, then add layers to a ggplot.
...
We can also add the data points to the plot to see how well the model fits the data. It gets messy, so it's easier to see faceting by `Treatment`.
...
- whether the % cover of bare rock varied among treatments and with time (Day) (**main effects**) 
- whether the slopes of the bare rock vs day relationships varied among treatments (**Treatment x Day interaction**).
...
For your final challenge today, we want you to test for **differences in relative `bill depth` between `species`**. 

We know that as individual penguins grow larger, their beaks get both longer and deeper. We want to know if there are differences in bill depth between species after we account for bill length. That is, do the bill depths between species differ when you hold bill length constant? To do this, we will run an ANCOVA using bill length as a covariate.
...
- **estimate means** for both **categorical predictors**
- produce a **plot** summarising your findings
- **estimate contrasts** to compare at a common size
...
# Estimate contrasts by species
estimate_contrasts(fit_bill, contrast = "species")

# Check depth to length ratio
# data_penguins |> 
#   mutate(
#     depth_to_length = bill_depth_mm/bill_length_mm
#   ) |> 
#   ggplot(aes(y = depth_to_length, x = body_mass_g, color = species)) +
#   geom_point() +
#   theme_classic()
```
...
If we did not account for bill length as a covariate, would our conclusions be the same? `r cloze("different", c("same", "different"))`. 
...
How much does the explained variance (R^2) in bill depth across individuals increase with the addition of bill length as a covariate? `r cloze("8.99%", c("1.52%", "8.99%", "15.32%"))`

```{r solution-3}
summary(fit_bill) # 0.7697
summary(fit_simple) # 0.6798
0.7697 - 0.6798 # 0.0899
```
