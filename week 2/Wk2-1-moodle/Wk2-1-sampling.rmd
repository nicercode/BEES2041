...
The natural world is variable. We can't measure everything, so we take samples. We use these samples to estimate the mean and variance of a quantity for the population. In future weeks we'll be looking at how the mean of a population changes with some predictor, x. But in today's practical we'll be looking at how we can estimate the mean of a population, and how confident we can be in our estimate. You should have covered this in MATH1041, but we'll go over it again here, as these are core fundamentals underpinning most statistical analyses you'll undertake during your schooling and future work. <!-- ![Artwork by @allison_horst](nice-R.png){width=70%} --> <br>
...
You can read more about Davey and his research [here](https://www.unsw.edu.au/science/our-schools/bees/about-us/our-people/postgraduate-research-students/davey-dor-grs-profile), and about emrgence times in his journal article at the *Proceedings of the Royal Society* [here](https://royalsocietypublishing.org/doi/10.1098/rspb.2024.1702).

The first challenge a baby turtle faces after hatching is to reach the surface of the sand. Davey thought it would be good to know roughly how long this takes. Some people said it took a really long time. However, he couldn't find any data on this. So he decided to collect some data himself. He collected data on 10 baby turtles, using some collar accelerometers he attached to the turtles as they hatch. This involved hatching eggs, attaching the collars, and then buyring the turtles again, as though they had just emerged from the nest.

![Image: Davey Dor](pics/turtle.jpeg)

Davey now wants to know the average time it takes for baby turtles to reach the surface.

He could of course just take the average of the 10 turtles he has measured, but he knows that this is just a sample of the population of baby turtles. So he wants to know how confident he can be that the average time he has calculated is the true average time for all baby turtles.
...
Before looking at Davey's data, let's briefly review some of the key concepts about probability, that underpin the estimation of confidence intervals.
...
-   the mean, $\mu$:, which is the centre of the distribution, and
...
**Question: What is the probability that the mean time it takes for baby turtles to reach the surface is less than or greater than some critical value, e.g. 20 minutes?**
...
A big advantage of the t-test is that you easily calculate an exact probability that the population mean you calculated is different to some critical value other than zero. This is harder to do with linear models.
...
And then calculate the 95% confidence intervals for the population mean
...
What are the mean and confidnece intervals for the school garden soil lead concentration?

-   mean: {1:NUMERICAL:=272.70:0.1}
-   min: {1:NUMERICAL:=230.09:0.1}
-   max: {1:NUMERICAL:=315.31:0.1}

Based on the CI, is there evidence that soils from this garden exceeded the safety guidelines? {1:MULTICHOICE_S:=No\~Yes}

## Palmer penguins

The Palmer penguins dataset contains data on the size of different penguins. A biologist was writing a field guide about the different species and thought to icnlude some infomation on the size of the penguins.
...
-   mean: {1:NUMERICAL:= 48.83:0.1}
-   min: {1:NUMERICAL:= 48.03:0.1}
-   max: {1:NUMERICAL:= 49.64:0.1}
...
-   mean: {1:NUMERICAL:= 38.79:0.1}
-   min: {1:NUMERICAL:= 38.36:0.1}
-   max: {1:NUMERICAL:= 39.22:0.1}

Based on the CIs, is there evidence that the bill length of the Adelie penguins is different to the bill length of the Chinstrap penguins? {1:MULTICHOICE_S:=Yes\~No}
...
data_Gentoo <- penguins |> filter(species == "Adelie")
```
