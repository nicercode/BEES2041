...
Some of the variables are % cover (the red, brown and green algae) and some are counts of individual animals. 100% cover is not equivalent to 100 snails, so we can standardise these variables to ensure they are on the same scale.
...
To do this for the heavy metal data set, we would use `hclust()`:
