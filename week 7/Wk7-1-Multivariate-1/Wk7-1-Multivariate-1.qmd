...
rprojroot::has_file("BEES2041-code.Rproj") |>
  rprojroot::find_root() |>
  file.path("week 7/Wk7-1-Multivariate-1") |>
  setwd()
...
# 7-1 Multivariate methods

## Introduction

Multivariate analyses are techniques used to explore or analyse data that involve **multiple variables at once**, helping to understand **relationships among them**. In the environmental sciences, for example, multivariate methods are commonly used to visualise differences in **species composition** across sites, or understand relationships among many correlated abiotic variables to describe sites or samples.

This week's practical provides an introduction to some of the graphical and analytical techniques used for multivariate data sets (those with more than one dependent variable).

Today we will focus on two graphical techniques: 
...
-   Create and interpret a **multi-dimensional scaling (MDS)** plot
...
We'll be using a new package, called `vegan`, which includes a range of tools for visualising and analysing community data in ecology, but useful for all sorts of multivariate data sets. Uncomment the last line of code and install it on your machine.
...
| :----------------------- | :------ | :------ | :------ |
| mean sediment size (mm)  | 4.00    | 2.00    | 0.80    |
| beach slope (degrees)    | 10.10   | 5.20    | 1.20    |
| mean wave height (m)     | 1.85    | 1.47    | 0.39    |
| mean current speed (m/s) | 1.20    | 0.80    | 0.70    |
...
## What are the samples and what are the variables?
...
## Summarise the relationships among samples using the Pearson correlation coefficient
...
## Summarise the relationships among samples using Euclidean distance

The Euclidean distance for each pair of samples, $y_1$ and $y_2$, is defined as:

$$
d(y_1,y_2) = \sum_{j=1}^{p}\sqrt{(y_{1j} - y_{2j})^2}
$$

where $y_{1j}$ and $y_{2j}$ are the values of variable $j$ in samples 1 and 2; $p$ is the number of variables.
...
This is a **dissimilarity matrix**, using a different measure (**Euclidean distance**) to quantify the differences (instead of similarity) among beaches.

**Questions:**

- What is the dissimilarity between Beach 1 and 2 as defined by Euclidean distance? 
...
Read the data set, `HeavyMetals.csv`, into R and have a look at its structure (samples as rows, variables as columns).
...
The first column is the name of each site. We don’t want to use the site numbers in our multivariate analysis, so we need to remove the Site variable using `select()`
...
Before we calculate the similarity among sites, we will perform a **square root transformation** on the data using `sqrt()`. This will **reduce the influence of very large values** (more on the reasons for transformations later in this practical).

```{r}
# Square root every value in the data
HeavyMetals_sq <- sqrt(HeavyMetals_vars)
```

**Compute similarity matrix**

Now we can use the `dist()` function again to create the **dissimilarity matrix**, using the **Euclidean distance** as our similarity coefficient.

```{r}
# Compute dissimilarity matrix using Euclidean distance
HeavyMetals_dis <- dist(HeavyMetals_sq, method = "euclidean")
```

The numbers in this **dissimilarity matrix** are then used to create a **multi-dimensional scaling** (MDS) plot to visualise the differences among sites. These are introduced in the lectures and you can also read the short tutorial on [Environmental Computing](https://environmentalcomputing.net/graphics/multivariate-vis/mds/).

**Multi-dimensional scaling**

We use the function `metaMDS()` from the package `vegan` to create an MDS plot.

```{r}
HeavyMetals_mds <- metaMDS(HeavyMetals_dis, autotransform = FALSE, trace = FALSE)
```

- the `autotransform` argument tells the function not to further transform the data
- the `trace` argument stops it from giving us all the details of the process trying to find the best solution.
...
```{r, results='hide'}
# Extract the points
Metals_xy <- data.frame(HeavyMetals_mds$points)
...
`r cloze("a measure of how well the distance between samples on the plot corresponds with the actual multivariate distance (the lower, the better)", c("a measure of how well the distance between samples on the plot corresponds with the actual multivariate distance (the lower, the better)", "a measure of how well the variables are correlated", "a measure of whether the data are normally distributed", "a measure of how close together all the samples are on the plot", "a statistical test contrasting samples in 2D space"))`

> Let's now interpret the plot with the very simple rule that those **points that are closest together are the most similar**.

## Which site is least similar to any other site?

`r cloze("S1", c("S1", "S9", "S6", "S2"))`

## Can you see any patterns in the similarity among sites that you can relate to the sampling design? Which sites are most similar to the dump site? {type=essay}
...
First, read in the data file `NutrientEnrichment.csv`
...
**Calculate dissimilarity and perform MDS**

Instead of creating a dissimilarity matrix and doing the MDS in two steps as you did for the heavy metals data, the function `metaMDS()` is able to compute dissimilarities and perform MDS directly from the dataframe in one function. 

```{r}
Nutrients_mds <- metaMDS(Nutrients_sq, 
                         distance = "bray", 
                         autotransform = FALSE, 
                         trace = FALSE) 
```
...
```{r, results='hide'}
# Extract points
Nutrients_xy <- data.frame(Nutrients_mds$points)
...
## Does the ordination distinguish between experimental treatments? Describe the patterns in the plot {type=essay}

# Maroubra

## Species composition in rock pools
![](images/rock_pool.PNG){width=70%} <br>

On the Maroubra field trip, multivariate data were collected to investigate the species composition of the intertidal community living in rock pools and neighbouring rocky habitats that were emergent at low tide.
...
```{r}
# Load data
Rock_pools <- read_csv("data/Maroubra_Rock_pools.csv")
```
...
Using the same methods as for the previous questions, produce a MDS plot to visualise differences in species composition between habitats (rock pool vs emergent).
...
```{r}
Rock_pools_vars <- Rock_pools |> select(-class, -group_id, -plot_number, -habitat)
```
...
We can use the function `decostand()` from `vegan` to **standardise** each variable by the maximum value of that variable. This function will give us variables with values between 0 and 1. No need to do any other transformations!
...
```{r solution}
# Run a MDS 
Rock_pools_mds <- metaMDS(Rock_pools_vars_std, distance = "bray", autotransform = FALSE, trace = FALSE)

# Extract points
Rock_pools_xy <- data.frame(Rock_pools_mds$points)
...
# Check stress
Rock_pools_mds$stress
```

## Does the ordination plot suggest any differences in species composition between rock pools and neighbouring emergent habitats? {type=essay}

## How confident are you that the MDS plot effectively represents the relationships among all samples in the multivariate data set? {type=essay}
...
You first explored the patterns in heavy metal pollution in this data set with an MDS ordination plot. 
...
```{r}
HeavyMetals_cluster <- dist(HeavyMetals_sq, method = "euclidean") |> hclust(method = "single")
```

Let's talk through this code: 

- `HeavyMetal_sq` is the square root transformed data we created earlier. 
...
```{r, results='hide'}
as.dendrogram(HeavyMetals_cluster) |> plot()
```
...
```{r, results='hide'}
HeavyMetals_cluster_trans <- 
  t(HeavyMetals_vars) |> 
  dist(method = "euclidean") |> 
  hclust(method = "single")

as.dendrogram(HeavyMetals_cluster_trans) |> plot()
```

## Concentrations

The concentrations of which two metals are most similar to each other in terms of Euclidean distance?

`r cloze("Co and Cd", c("Co and Cd", "Mn and Zn", "Cu and Pb", "Ni and Co"))`

## Does the cluster analysis suggest any effects of the sewage dump {type=essay}

## Transforms

## To transform or to standardise?

Multivariate data sets are often **transformed** prior to making plots. This is not to meet the assumptions of a statistical test (these plots are not hypothesis tests), but **to standardise the relative importance of variables** that may be measured on **different scales**.
...
```{r}
HeavyMetals_standardised <- decostand(HeavyMetals_vars, method = "standardize")
```
...
```{r, results='hide'}
dist(HeavyMetals_standardised, method = "euclidean") |> 
  hclust(method = "single") |> 
  as.dendrogram() |> 
  plot()
```

## Make some notes on how the relationships between sites are altered by standardising the variables {type=essay}
