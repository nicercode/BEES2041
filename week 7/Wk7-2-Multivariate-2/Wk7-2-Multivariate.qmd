...
rprojroot::has_file("BEES2041-code.Rproj") |>
  rprojroot::find_root() |>
  file.path("week 7/Wk7-2-Multivariate-2") |>
  setwd()
...
In this practical, we will continue our look at methods to analyse and visualise complex **multivariate data sets** - ones where **more than one response variable has been measured from each replicate sample**.
...
## multivariate hypothesis testing: Nutrient enrichment experiment
...
Instead of partitioning the variation in the actual values of the variables (i.e., abundance of each of the weed species), as in a ANOVA, this analysis **partitions the variation in the values of the similarity matrix** among and within treatment groups.

Another way to think of this is that **if there was a strong effect of treatment, then all the replicates within each level of treatment are more likely to be similar to each other than they would be to replicates from another level** (i.e., within-group similarities higher than among-group similarities).
...
# Select weeds only
Nutrients_vars <- Nutrients |> select(contains("Weed"))

# Transform data
Nutrients_sq <- sqrt(Nutrients_vars)
```

**Run a PERMANOVA**

We will use the function `adonis2()` in `vegan`. This function performs an ANOVA on the similarity matrix of your choosing:
...
-  the analysis is run on similarities among samples, not the actual values of the original variables
...
permutations <- permustats(Nutrients_test)$permutations |> 
  as_tibble() |>
  rename(permutation_outcome = V1)

ggplot(data = permutations, aes(x = permutation_outcome)) +
  geom_density() +
  geom_jitter(aes(y = 0), height = 0.05, width = 0, color = "darkblue", alpha = 0.2) +
  geom_vline(xintercept = permustats(Nutrients_test)$statistic, color = "red", linetype = "dashed") +
  theme_classic() +
  labs(y = "Density")
```

This produces a frequency histogram of `pseudo-F` obtained from the 999 permutations of the raw data. **This is best viewed as what would happen if the null hypothesis was true** (no similarities between our variables). The `pseudo-F` value you obtained is noted with the vertical red line.
...
Our next step is to try and understand **which treatment differed from which**, i.e., contrasts. There is unfortunately no function to do this, but a workaround is to run separate pairwise tests using subsets of data with only two levels of `Treatment`. 

We will use `filter()` to subset rows, and `select()` for subsetting columns for our pairwise comparisons.  The following code creates subsets of rows for each comparison (note that `!=` means “not equal to”) and runs the **permutational MANOVA** again with just the rows with the weed variables.

**Subset analysis for Control and Low**

```{r, results='hide'}
# Filter treatment is not "High"
Nutrients_ControlvsLow <- Nutrients |> filter(Treatment != "High")

# Select columns containing the word "weed"
Nutrients_ControlvsLow_vars <- Nutrients_ControlvsLow |> select(contains("Weed"))
...
Nutrients_ControlvsHigh <- Nutrients |> filter(Treatment != "Low")

# Select columns containing the word "weed"
Nutrients_ControlvsHigh_vars <- Nutrients_ControlvsHigh |> select(contains("Weed"))
...
Nutrients_LowvsHigh <- Nutrients |> filter(Treatment != "Control")

# Select columns containing the word "weed"
Nutrients_LowvsHigh_vars <- Nutrients_LowvsHigh |> select(contains("Weed"))

# Perform a PERMANOVA using subset data
adonis2(Nutrients_LowvsHigh_vars ~ Nutrients_LowvsHigh$Treatment, permutations = 999, method = "bray")
```

The problem with this approach is increasing the risk of **false positives**. False positives are also called **type 1 errors**. The idea is, with increasing numbers of pairwise tests being run, by chance, we will detect a statistically significant treatment difference even though the underlying populations may not be different. 

![](images/type_1_errors.png){width=60%} <br>

The simplest correction for this is called the **Bonferroni correction**, where the significance level (α = 0.05) is **adjusted by the number of hypotheses being tested**. 

In this example, we will **divide the threshold significance value by three (we have three different treatments)** before interpreting which comparisons are significant (in other words, α for each test becomes `0.05/3`).

## What do the multivariate tests and their associated probabilities tell you about the differences in species composition among nutrient treatments? Use the pairwise tests to establish which treatments differ from which. {type=essay}


## Species composition in rock pools (multivariate hypothesis testing)

![](images/rock_pool.PNG){width=70%} <br>

For your practical report, you want you to accompany the text and figures that contrast species composition in and out of rockpools, with a formal statistical test. You can use a **permutational multivariate analysis of variance** for this, modifying code that you just used for the nutrient enrichment experiment.
...
Research at UNSW by [Professor David Warton's group](http://www.eco-stats.unsw.edu.au/) is focused on developing methods to better analyse these sorts of data, and they have released an R package 'mvabund' that uses model-based approaches to analyse multivariate data sets. David has even promoted his package with a [video](https://youtu.be/KnPkH6d89l4) drawing on the considerable talents of 80's pop star Rick Astley!
...
Leaf_shape_vars <- Leaf_shape |> select(-Species)
...
## What makes this data set particularly suitable for PCA?

`r cloze("there are multiple variables that are correlated with each other", c("there are lots of replicate samples", "there are multiple variables measured", "there are two groups of samples that we want to contrast", "there are multiple variables that are correlated with each other"))`
...
```{r}
leaf_pca <- princomp(Leaf_shape_vars, cor = FALSE)
```

Let's talk through the code: 

- `cor = FALSE` indicates you want to use the covariance matrix, which is appropriate when all variables are measured on a similar scale or have a similar range of values. Change to `cor = TRUE` to scale the variables and use a correlation matrix (recommended when all the variables are on different scales).

Principal components analysis produces a lot of graphical and numerical output, which we can explore by further examining the object (`leaf_pca`) we just created.
...
```{r, results='hide'}
plot(leaf_pca$scores, pch = 16) # `pch` controls the symbol used for plotted points
```
...
PCA is aiming to **represent as much variance as possible in the first few principal components**. We can check whether it is doing a good job by exploring the `summary()` of the `leaf_pca` object.

```{r, results='hide'}
summary(leaf_pca)
```

## Enter the proportion explained by the first two principal components

- PC1: `r cloze_numerical(0.9, tolerance = 0.01)` 
- PC2: `r cloze_numerical(0.06, tolerance = 0.01)`

**Visualise variance explained by principal components**
...
## Which of the original variables is most strongly correlated with principal component 1?
...
Leaf_xy <- data.frame(leaf_pca$scores)
```

Add the labels for the two species to this data

```{r}
# Extract species names from data and add to Leaf_xy
...
## Does the principal components analysis suggest any size or shape differences between the leaves of the two species? {type=essay}

## Using the coefficients and the loadings, which variables are predominantly associated with the separation of species A and B.

`r cloze("Width 1 and Width 2", c("Only Width 1", "Width 1 and Width 2", "Width 1 and Total length", "Total length and Petiole length"))`

# Heavy metals in sediments (PCA)
...
HeavyMetals_vars <- HeavyMetals |> select(-Site)
```
...
```{r solution}
# Perform PCA
std_HM <- decostand(HeavyMetals_vars, method = "max")
std_HM 

metals_pca <- princomp(std_HM, cor = FALSE)

# Extract loadings from PCA
loadings(metals_pca)

plot(metals_pca$scores, pch = 16) # `pch` controls the symbol used for plotted points
biplot(metals_pca)

# Without standardising first
metals_pca <- princomp(HeavyMetals_vars, cor = TRUE)
...
## Which variable (i.e., metal) is most strongly correlated with the values of PC1?

`r cloze("Cr", c("Cu","Mn","Co","Ni","Zn","Cd","Pb","Cr"))`

## Comparing all your classification or ordination plots, discuss which technique best visualises the patterns in metal contamination among sites. Do any clearly identify sites close to the sewage dump? {type=essay}


## Make an interactive plot of the dataset. Try visualise Ni, Cr, Pb on the x, y, z axis. How does this interactive plot compare with your ordination from last prac? (Try standardising your variables first!)
...
To understand the data set, plot the patterns in drip numbers through time for just one of the loggers (from columns `HW-2b`, `HW_4b`, etc.). Let's first format the date column as a date, not character so that R handles years and dates nicely. 
...
Now let's visualise the patterns in drip numbers through time for just one of the loggers (`HW_4b`).
...
## Using PCA to visualise variation among years 
...
- Bring back the year column into the data frame that has the x and y coordinate values from the ordination plot.
...
# Perform PCA
caves_pca <- princomp(Caves_vars, cor = FALSE)
...
Caves_xy <- data.frame(caves_pca$scores)
